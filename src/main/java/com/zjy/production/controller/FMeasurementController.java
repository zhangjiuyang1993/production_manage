...
public class FMeasurementController {
	 ...
	 *
	 */
    @RequestMapping("/get/{finalMeasureCheckId}")
    @ResponseBody
    public FinalMeasuretCheck getItemById(@PathVariable String finalMeasureCheckId) throws Exception{
        FinalMeasuretCheck finalMeasuretCheck = measureService.get(finalMeasureCheckId);
    ...
    @ResponseBody
    public CustomResult insert(@Valid FinalMeasuretCheck finalMeasuretCheck, BindingResult bindingResult) throws Exception{
        CustomResult result;
        ...
        if(measureService.get(finalMeasuretCheck.getfMeasureCheckId()) != null){
            result = new CustomResult(0,"该设备计量质检编号已经存在，请更换编号",null);
        }else{
    ...
    @ResponseBody
    public CustomResult updateAll(@Valid FinalMeasuretCheck finalMeasuretCheck,BindingResult bindingResult) throws Exception{
        if(bindingResult.hasErrors()){
            FieldError fieldError = bindingResult.getFieldError();
            return CustomResult.build(100,fieldError.getDefaultMessage());
        }
    ...
    @ResponseBody
    public CustomResult updateNote(@Valid FinalMeasuretCheck finalMeasuretCheck,BindingResult bindingResult) throws Exception{
        if(bindingResult.hasErrors()){
            FieldError fieldError = bindingResult.getFieldError();
            return CustomResult.build(100,fieldError.getDefaultMessage());
        }
    ...
    @ResponseBody
    public EUDataGridResult searchFMeasureCheckByFMeasureCheckId(Integer page,Integer rows,String fMeasureCheckId) throws Exception{
        EUDataGridResult result = measureService.searchFMeasureCheckByFMeasureCheckId(page,rows,fMeasureCheckId);
        return result;
    }
    ...
    public EUDataGridResult searchFMeasureCheckByOrderId(Integer page,Integer rows,String orderId) throws Exception{
        EUDataGridResult result = measureService.searchFMeasureCheckByOrderId(page,rows,orderId);
        return result;
    }
}
