...
import com.zjy.production.domain.Custom;
import com.zjy.production.domain.CustomExample;
import com.zjy.production.domain.customize.CustomResult;
import com.zjy.production.domain.customize.EUDataGridResult;
import com.zjy.production.mapper.CustomMapper;
...
public class CustomServiceImpl implements CustomService {
	...
	public EUDataGridResult searchCustomByCustomName(int page, int rows, String customName) throws Exception {
		//分页处理
		PageHelper.startPage(page, rows);
		List<Custom> list = customMapper.searchCustomByCustomId(customName);
		//创建一个返回值对象
		EUDataGridResult result = new EUDataGridResult();
		result.setRows(list);
		//取记录总条数
